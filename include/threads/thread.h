...
struct thread {
	...
	struct list_elem elem_donate;
	struct list_elem elem_sleep;
	struct list_elem elem_all;

	struct list_elem elem_in_parent;

	bool donators_initialized;
	struct lock* donated_for;

	struct condition exited;
	struct lock for_exited;
	struct semaphore child_creation;
	//tid_t child_creation_tid;
	
	struct list donators;
	struct list child_threads;
	struct list fds;

	int64_t tick_started_sleeping;
